...
func NewTripConsumer(rabbitmq *messaging.RabbitMQ, service service.PaymentService) *TripConsumer {
	return &TripConsumer{
		rabbitmq: rabbitmq,
		service:  service,
	}
...
func (c *TripConsumer) Listen() error {
	return c.rabbitmq.Consume(messaging.PaymentTripResponseQueue, func(ctx context.Context, msg amqp091.Delivery) error {
...
func (c *TripConsumer) handleTripAccepted(ctx context.Context, payload messaging.PaymentTripResponseData) error {
	...
	if err := c.rabbitmq.Publish(ctx, contracts.PaymentEventSessionCreated,
